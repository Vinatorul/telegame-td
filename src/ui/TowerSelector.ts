...
export class TowerSelector {
  ...
  private createButtons(): Phaser.GameObjects.Container {
    const container = this.scene.add.container(20, 500);
    
    const bg = this.scene.add.rectangle(0, 0, 200, 80, 0x333333, 0.8);
    bg.setOrigin(0, 0);
    container.add(bg);
  ...
  private createTowerButton(
    ...
    const cost = this.scene.add.text(0, size / 2 + 5, `${this.towerCosts[towerType]}`, {
      fontSize: '12px',
      color: '#ffffff'
    });
